...
_curPosition = position player;

// Get closest airport
_airportCfgName = [_curPosition] call mav_planetaxi_fnc_getClosestAirport;
_localPos = (getArray (missionConfigFile >> "Maverick_PlaneTaxis" >> "Airports" >> worldName >> _airportCfgName >> "entryPosition")) select 0;

// Give NPC/object addActions
// Get all airport configs
_configs = "true" configClasses (missionConfigFile >> "Maverick_PlaneTaxis" >> "Airports" >> worldName);
_ppm = getNumber (missionConfigFile >> "Maverick_PlaneTaxis" >> "Costs" >> "pricePerMeter");


{
	if ((configName _x) != _airportCfgName) then {
		_name = getText (missionConfigFile >> "Maverick_PlaneTaxis" >> "Airports" >> worldName >> configName _x >> "displayName");
		_posData = getArray (missionConfigFile >> "Maverick_PlaneTaxis" >> "Airports" >> worldName >> configName _x >> "entryPosition");
